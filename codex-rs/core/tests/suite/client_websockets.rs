...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_websocket_statusless_error_is_terminal() {
    skip_if_no_network!();
    ...
    let error = tokio::time::timeout(Duration::from_secs(1), stream.next())
        .await
        .expect("status-less error event should terminate immediately")
        .expect("stream should emit the terminal error")
        .expect_err("status-less error event should not be treated as progress");
    assert!(
        error
            .to_string()
            .contains("An internal server error occurred.")
    );
