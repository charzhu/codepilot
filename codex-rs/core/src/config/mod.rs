...
pub struct Config {
    ...
    /// TUI appearance skin override (kebab-case id).
    pub tui_skin: Option<String>,
...
impl Config {
    pub fn multi_agent_v2_enabled(&self) -> bool {
        self.multi_agent_version_from_features() == MultiAgentVersion::V2
    }
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        for (name, server) in self.mcp_servers.get() {
            configured_mcp_servers.insert(name.clone(), server.clone());
        }
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let config = Self {
            ...
            tui_skin: cfg.tui.as_ref().and_then(|t| t.skin.clone()),
...
fn apply_external_mcp_discovery(
    ...
    let plugin_mcp_owners: Vec<(String, String)> = plugin_ids_by_mcp_server_name
        .iter()
        .map(|(name, plugin_id)| (name.clone(), format!("plugin:{plugin_id}")))
        .collect();
