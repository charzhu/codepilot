...
#[tokio::test]
async fn unfinished_startup_prewarm_does_not_block_the_first_turn() {
    let (sess, _) = make_session_and_context().await;
    let (startup_prewarm_tx, startup_prewarm_rx) = tokio::sync::oneshot::channel::<()>();
    let handle = tokio::spawn(async move {
        let _ = startup_prewarm_rx.await;
        Ok(test_model_client_session())
    });
    ...
    .await
    .expect("unfinished startup prewarm should not block the first turn");
    ...
    assert!(
        startup_prewarm_tx.send(()).is_err(),
        "unfinished startup prewarm task should be aborted"
    );
}
