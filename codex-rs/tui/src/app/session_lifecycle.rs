...
impl App {
    /// Refreshes liveness for all tracked agent threads.
    ///
    /// Returns `true` if a "running agents" status cell was shown for path-backed subagent
    /// threads in place of the full picker, in which case the caller should skip opening it.
    async fn refresh_agent_picker_liveness(&mut self, app_server: &mut AppServerSession) -> bool {
    ...
    pub(super) async fn open_agent_picker(&mut self, app_server: &mut AppServerSession) {
        if self.refresh_agent_picker_liveness(app_server).await {
            return;
        }
