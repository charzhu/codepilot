...
pub fn process_responses_event(
    ...
    match event.kind.as_str() {
        ...
        "error" => {
            return Err(ResponsesEventError::Api(ApiError::Stream(
                "error event received from server".into(),
            )));
        }
        _ => {
...
mod tests {
    ...
    #[test]
    fn top_level_error_event_is_terminal() {
        ...
        .expect("parse top-level error event");

        let result = process_responses_event(event);
        match result {
            Err(ResponsesEventError::Api(ApiError::Stream(message))) => {
                assert_eq!(message, "error event received from server");
            }
