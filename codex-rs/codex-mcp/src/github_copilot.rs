...
pub const GITHUB_COPILOT_MCP_SERVER_URL: &str = "https://api.enterprise.githubcopilot.com/mcp";
const GITHUB_COPILOT_MCP_TOOLS: [&str; 48] = [
...
mod tests {
    ...
    #[test]
    fn github_copilot_mcp_server_config_requests_full_union_toolset() {
        ...
        assert_eq!(
            http_headers
                ...
                .map(String::as_str),
            Some(super::GITHUB_COPILOT_MCP_TOOLS.join(",").as_str())
        );
        assert_eq!(
            config.enabled_tools,
            Some(
                super::GITHUB_COPILOT_MCP_TOOLS
                    .iter()
                    .map(ToString::to_string)
                    .collect()
            )
        ...
        assert!(
            config
                .enabled_tools
                .as_ref()
                .is_some_and(|tools| tools.contains(&"web_search".to_string()))
        );
        assert!(
            config
                .enabled_tools
                .as_ref()
                .is_some_and(|tools| tools.contains(&"push_files".to_string()))
        );
    }
}
