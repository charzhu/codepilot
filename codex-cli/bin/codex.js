...
// Codepilot ships its own platform package for Windows x64 only today; the
// table mirrors the upstream Codex layout so it is easy to add new triples
// later without diverging structurally from upstream.
const PLATFORM_PACKAGE_BY_TARGET = {
  "x86_64-pc-windows-msvc": "@charzhu/codepilot-win32-x64",
};
...
function findCodexExecutable() {
  ...
  const updateCommand =
    packageManager === "bun"
      ...
      : packageManager === "pnpm"
        ? "pnpm add -g @charzhu/codepilot@latest"
        : "npm install -g @charzhu/codepilot@latest";
  throw new Error(
    `Missing optional dependency ${platformPackage}. Reinstall Codepilot: ${updateCommand}`,
  );
}
...
function isPnpmOwnedCodexInstall(nodeModulesDir) {
  ...
  try {
    return (
      realpathSync(path.join(nodeModulesDir, "@charzhu", "codepilot")) ===
      codexPackageRoot
    );
  } catch {
    return false;
